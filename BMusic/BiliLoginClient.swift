...
final class BiliLoginClient {
    ...
    func acceptWebLoginCookie(_ cookie: String) async throws -> Any {
        guard try await isValidLoginCookie(cookie) else {
            throw BiliLoginError.invalidWebLoginCookie
        }

        try cookieStore.write(cookie)
        return try await fetchUserInfo(cookieOverride: cookie)
    }
